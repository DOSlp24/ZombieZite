...
class ZombieDeckSpec extends WordSpec with Matchers {
  ...
  "A Zombie Deck" can {
    ...
    "return a Schlürfi" in {
      zd.getDrawnZombieR(0) should be(Zombie(a, "Schlurfer", 19, 0, 100))
    }
    "return a Runner" in {
      zd.getDrawnZombieR(1) should be(Zombie(a, "Runner", 9, 0, 70))
    }
    "return a Fatti" in {
      zd.getDrawnZombieR(2) should be(Zombie(a, "Fatti", 39, 0, 250))
    }
    "return a Tank" in {
      zd.getDrawnZombieR(3) should be(Zombie(a, "Tank", 49, 1, 500))
    }
    "return a Spitter" in {
